...
#define TRI_INIT_CALLBACK_IMPL(callbackName, name) void TRI_CONCAT(name, _func)(); bool TRI_CONCAT(name, _var) = [](){ Environment::init(); env->events->init.addCallback(callbackName, [](){TRI_CONCAT(name, _func)();}); return true;}(); void TRI_CONCAT(name, _func)()
#define TRI_INIT_CALLBACK(callbackName) TRI_INIT_CALLBACK_IMPL(callbackName, TRI_UNIQUE_IDENTIFIER(_tri_register_callback))
#define TRI_UPDATE_CALLBACK_IMPL(callbackName, name) void TRI_CONCAT(name, _func)(); bool TRI_CONCAT(name, _var) = [](){ Environment::init(); env->events->update.addCallback(callbackName, [](){TRI_CONCAT(name, _func)();}); return true;}(); void TRI_CONCAT(name, _func)()
...
#define TRI_SHUTDOWN_CALLBACK_IMPL(callbackName, name) void TRI_CONCAT(name, _func)(); bool TRI_CONCAT(name, _var) = [](){ Environment::init(); env->events->shutdown.addCallback(callbackName, [](){TRI_CONCAT(name, _func)();}); return true;}(); void TRI_CONCAT(name, _func)()
